	...
	for (const platform in names) {
		...
		if (!names[platform].uniqueID) {
			...
			names[platform].uniqueID = {
				en: `${manufacturer}: ${englishFullName}`
			};
		}
		if (!names[platform].postScriptName) {
			names[platform].postScriptName = {
				en: postScriptName
			};
		}
	}
		...
		if (hasBlend || hasPoints) {
			...
			if (hasPoints) {
				...
				if (variationData) {
					...
					let transformedPoints = this.applyTupleVariationStore(variationData, glyphPoints, coords, "gvar", {
						glyph
					});
		...
		for (const rule of ruleSet) {
			...
			if (matched) {
				...
				const parser = (substitutions2, lookupRecord) => {
					...
					const {
						subtables
					} = this.getLookupByIndex(lookupListIndex);
